...
def dataload(dataset):
    samples=[]
    for s1,s2,score in zip(dataset["sentence1"],dataset["sentence2"],dataset["similarity_score"]):
        score = float(score) / 5.0  # Normalize score to range 0 ... 1
        inp_example = InputExample(texts=[s1, s2], label=score)
...
train_samples=dataload(train_dataset)
test_samples=dataload(test_dataset)
dev_samples=dataload(dev_dataset)
